...
BASELINE_ONLY = False  # set False to also train teacher + distilled student
...
# Teacher architecture (LibriBrain-style)

class ResNetBlock(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.conv1 = nn.Conv1d(channels, channels, 3, padding = "same")
        self.conv2 = nn.Conv1d(channels, channels, 1, padding = "same")

    def forward(self, x):
        return self.conv2(self.conv1(x)) + x


class TeacherPhonemeModel(pl.LightningModule):
    def __init__(self, num_classes = 39, dropout_p = 0.5):
        super().__init__()
        self.model = nn.Sequential(
            nn.Conv1d(306, 128, 7, padding = "same"),   # 1
            ResNetBlock(128),                            # 2a + 2b
            nn.ELU(),                                    # 3
            nn.Conv1d(128, 128, 50, stride = 25),       # 4  → (128, 4)
            nn.ELU(),                                    # 5
            nn.Conv1d(128, 128, 7, padding = "same"),   # 6
            nn.ELU(),                                    # 7
            nn.Flatten(),                                # 8  → 512
            nn.Linear(512, 512),                         # 9
            nn.ReLU(),                                   # 10
            nn.Dropout(dropout_p),                       # 11
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_f1_macro",     self.f1_macro(y_hat, y),  prog_bar = True, on_step = True, on_epoch = True)
        self.log("train_balanced_acc", self.balanced_acc(y_hat, y), prog_bar = True, on_step = True, on_epoch = True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_f1_macro",     self.f1_macro(y_hat, y),  prog_bar = True)
        self.log("val_balanced_acc", self.balanced_acc(y_hat, y), prog_bar = True)
        return loss
...
# Define base phoneme model (student / baseline architecture)
...
if not BASELINE_ONLY:
    ...
    teacher_ckpt = f"{base_path}/models/phoneme_teacher_libribrain.ckpt"
    ...
    if os.path.exists(teacher_ckpt):
        ...
        teacher_model = TeacherPhonemeModel.load_from_checkpoint(teacher_ckpt)
    else:
        print("Training teacher model from scratch...")
        teacher_model = TeacherPhonemeModel(num_classes = len(train_dataset.labels_sorted))
